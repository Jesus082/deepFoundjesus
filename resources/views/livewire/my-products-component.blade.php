    ...
    <div class="">
        ...
        <div class="col-md-3 mb-3" id="botones-filtros">
            <div class="d-flex justify-content-between">
                <button wire:click="$set('saleStatus', '')">Todos</button>
                <button wire:click="$set('saleStatus', 'reserved')">Reservados</button>
                <button wire:click="$set('saleStatus', 'sold')">vendidos</button>
            </div>
        </div>
    </div>
        ...
        <div class="col-lg-3 col-md-4 col-sm-6 col-6 p-2 pb-5">
            <a href="{{ route('item', ['id' => $product->id]) }}" target="_blank">
                <div class="card preview-card-product" id="card_myproducts" wire:key="{{ $loop->index . rand()}}">
                    <x-products-card.preview-product-card :product="$product" :isUserProductsRoute="$isUserProductsRoute" />
                </div>
            </a>
        </div>
    @endforeach
